...
[t1,x1] = ode45(@(t,x) systemEqs(t,x,beta_1,beta_2,beta_3,d_1,d_2,d_3,mu,k,r, ...
    alpha, K,sigma_1,sigma_2,gamma_1,gamma_2,gamma_3,p_vals(1)),tspan,x0);

[t2,x2] = ode45(@(t,x) systemEqs(t,x,beta_1,beta_2,beta_3,d_1,d_2,d_3,mu,k,r, ...
    alpha, K,sigma_1,sigma_2,gamma_1,gamma_2,gamma_3,p_vals(2)),tspan,x0);

[t3,x3] = ode45(@(t,x) systemEqs(t,x,beta_1,beta_2,beta_3,d_1,d_2,d_3,mu,k,r, ...
    alpha, K,sigma_1,sigma_2,gamma_1,gamma_2,gamma_3,p_vals(3)),tspan,x0);
...
plot(t1,(x1(:,1) + x1(:,2) + x1(:,3)),'k',t2,(x2(:,1) + x2(:,2) + x2(:,3)),'r--', ...
    t3,(x3(:,1) + x3(:,2) + x3(:,3)),'b:',LineWidth=1);
xlabel('Time (in days)')
ylabel('Total bee population size')
ylim([0 20000])
legend('p = 0', 'p = 0.13', 'p = 0.14')
grid on
