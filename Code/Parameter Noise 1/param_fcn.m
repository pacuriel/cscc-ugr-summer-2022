% Funtion to get seasonal paramerter values for a given time span
...
% for loop to calculate parameter(t)
    for i = 1:numel(t)
        ...
        if (size(t,1) == 1) % tspan has size (1xn)
            curr_time = t(1,i);
        elseif (size(t,2) == 1) % t produced by ode45 has size (nx1)
            curr_time = t(i,1);
        end
        ...
        if ((0 <= curr_time) && (curr_time < 91.25)) % Spring
            parameter(i,1) = parameter_noise_2(parameter_vals(1,1));
        elseif ((91.25 <= curr_time) && (curr_time < 2*91.25)) % Summer
            parameter(i,1) = parameter_noise_2(parameter_vals(2,1));
        elseif ((2*91.25 <= curr_time) && (curr_time < 3*91.25)) % Fall
            parameter(i,1) = parameter_noise_2(parameter_vals(3,1));
        elseif ((3*91.25 <= curr_time) && (curr_time <= 4*91.25)) % Winter
            parameter(i,1) = parameter_noise_2(parameter_vals(4,1));
        end
    end
